...
import com.zkjinshi.superservice.pad.adapter.OrderAdapter;
import com.zkjinshi.superservice.pad.adapter.OrderMoreAdapter;
...
//                        loadOrderList(orderAdapter.getLastTimeStamp());
    ...
    private void getOrderList(String shopID, String userID, int page, int pageSize) {
        ...
        if(IdentityType.BUSINESS ==  CacheUtil.getInstance().getLoginIdentity()){
            url = ConfigUtil.getInst().getForDomain()+"order/list/shop/"+shopID+"/"+page+"/"+pageSize;
        }
        Log.i(TAG, url);
